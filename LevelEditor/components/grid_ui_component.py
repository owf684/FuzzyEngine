...
class GridUIComponent:
    ...
    def __init__(self):
        ...
        self.eox = 0
        self.scroll_delta = 0
        self.max_screen_width = self.screen_width - self.grid_size
        self.grid_color = (255, 255, 255)
        self.reset_scroll = False

    def draw_grid(self, screen):
        for x in range(0, int(self.screen_width) + int(abs(self.scroll_offset)), self.grid_size):
            pygame.draw.line(screen, self.grid_color, (x - self.scroll_offset, 0),
                             (x - self.scroll_offset, self.screen_height))
